...
def save_genuine_distances_to_file(genuine_pair_data, output_path):
    """
    Saves a list of genuine pairs and their Hamming distances to a text file.
    
    Args:
        genuine_pair_data (list): A list of tuples, each containing (file1, file2, distance).
    ...
    print(f"\nSaving genuine pair distances to {output_path}")
    with open(output_path, 'w') as f:
        f.write("Genuine Pair Comparisons\n")
        f.write("--------------------------------\n")
        f.write("{:<20} | {:<20} | {}\n".format("File 1", "File 2", "Hamming Distance"))
        f.write("--------------------------------\n")
        for file1, file2, distance in genuine_pair_data:
            f.write("{:<20} | {:<20} | {:.4f}\n".format(os.path.basename(file1), os.path.basename(file2), distance))
    print("Done.")
...
def create_and_compare_pairs(all_templates_dict):
    ...
    # List of images with improper segmentation, including the .jpg extension
    problem_images = [
        "028_2_4.jpg.mat", "031_2_4.jpg.mat", "061_1_1.jpg.mat", "062_1_1.jpg.mat", "062_1_2.jpg.mat", "062_1_3.jpg.mat",
        "062_2_1.jpg.mat", "062_2_2.jpg.mat", "062_2_3.jpg.mat", "062_2_4.jpg.mat", "063_1_1.jpg.mat", "063_1_2.jpg.mat",
        "063_2_4.jpg.mat", "090_1_1.jpg.mat", "090_1_3.jpg.mat", "090_2_2.jpg.mat", "090_2_3.jpg.mat", "090_2_4.jpg.mat",
        "004_1_1.jpg.mat", "011_1_2.jpg.mat", "016_2_4.jpg.mat", "018_2_1.jpg.mat", "020_1_1.jpg.mat", "028_1_3.jpg.mat",
        "091_1_1.jpg.mat", "091_1_2.jpg.mat", "091_1_3.jpg.mat", "091_2_1.jpg.mat", "091_2_2.jpg.mat", "091_2_3.jpg.mat",
        "091_2_4.jpg.mat"
    ]
    ...
    for subject_id in subjects:
        ...
        # Filter out problem images for genuine comparisons
        # This list will store tuples of (filename, (template, mask))
        valid_templates = []
        for file, data in templates_for_subject:
            basename = os.path.basename(file)
            if basename not in problem_images:
                valid_templates.append((file, data))
                # print(basename)
            else:
                print(f"Invalid template: {basename}")

        # Create pairs of all unique combinations of valid templates
        for i in range(len(valid_templates)):
            for j in range(i + 1, len(valid_templates)):
                file1, (temp1, mask1) = valid_templates[i]
                file2, (temp2, mask2) = valid_templates[j]
                ...
                if temp1 is not None and temp2 is not None:
                    ...
                    genuine_pair_data.append((file1, file2, distance))
                    
                    if genuine_pairs_printed < 5:
                        # Print an example pair for verification
                        print(f"  Genuine pair: {os.path.basename(file1)} vs {os.path.basename(file2)} | Distance: {distance:.4f}")
    ...
    # Create a flattened list of all VALID templates for random selection
    all_templates_list = []
    for s_id, temp_list in all_templates_dict.items():
        for file, temp_data in temp_list:
            basename = os.path.basename(file)
            if basename not in problem_images:
                all_templates_list.append((s_id, file, temp_data))
    ...
    for _ in tqdm(range(num_impostor_pairs)):
        ...
        # The fix is to unpack correctly:
        file1_impostor, (temp1, mask1) = pair_data1[1:]
        file2_impostor, (temp2, mask2) = pair_data2[1:]
        ...
        if temp1 is not None and temp2 is not None:
            ...
            if impostor_pairs_printed < 5:
                # Print an example pair for verification
                print(f"  Impostor pair: {os.path.basename(file1_impostor)} vs {os.path.basename(file2_impostor)} | Distance: {distance:.4f}")
                impostor_pairs_printed += 1
            
    return genuine_distances, impostor_distances, genuine_pair_data


if __name__ == '__main__':
    # parsing args from the terminal
    parser = argparse.ArgumentParser(description="Iris Template Comparison Pipeline")
    parser.add_argument("--template_dir", type=str, default="./templates/CASIA1/features",
    ...
    if n_templates == 0:
        ...
        for file in tqdm(template_files):
            ...
            if subject_id:
                try:
                    ...
                    # Store the file path along with the data so we can filter later
                    all_templates_dict[subject_id].append((file, (template, mask)))
                except Exception as e:
                    print(f"Error loading {file}: {e}")
        
        # --- Step 2: Create and compare pairs ---
        start_time = time()
        genuine_distances, impostor_distances, genuine_pair_data = create_and_compare_pairs(all_templates_dict)
        end_time = time()
        ...
        # --- Step 6: Save genuine comparison results to a file ---
        if genuine_pair_data:
            genuine_distances_path = os.path.join(args.output_plot_dir, "genuine_pair_distances.txt")
            save_genuine_distances_to_file(genuine_pair_data, genuine_distances_path)
